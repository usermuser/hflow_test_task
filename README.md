...
4. Необходимо реализовать функционал добавления(прикрепления) файла к отправке
    * мы храним путь до файла в переменной `fp` класса `Candidate`
   ...
   Возникла необходимость создать отдельный класс `Attachment`. Методы класса:  
   ```
   _get_attachments():
       """Бегаем по папкам, сохраняем пути до файлов"""
       pass
